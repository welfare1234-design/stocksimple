// Cloudflare Pages Function: Big holders data from norway.twsthr.info
...
export const onRequest: PagesFunction = async () => {
  try {
    ...
    const stocks: Array<{ code: string; name: string; bigHolderPct: string; change: string; prevPct: string }> = [];
    ...
    for (let i = 1; i < rows.length && stocks.length < 30; i++) {
      ...
      stocks.push({ code, name, bigHolderPct: currentPct, change, prevPct: prevNum.toFixed(2) });
    }
